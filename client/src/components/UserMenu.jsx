...
const user = useSelector((state) => state.user)
  ...
  <span className='text-medium text-red-500'>{user.role === "ADMIN" ? "(Admin)" : ""}</span>
        ...
        <Link onClick={handleClose} to={'/dashboard/profile'} className='hover:text-primary-200'><FaExternalLinkAlt size={15} /></Link>
      </div>
      <Divider />
      <div className="text-sm grid gap-2">
        {
          isAdmin(user.role) && (
           ...
           {
          isAdmin(user.role) && (
            <Link onClick={handleClose} to={"/dashboard/uplordproduct"} className='px-2 hover:bg-orange-200 py-1' >uplord Product</Link>
          )
        }
            {
          isAdmin(user.role) && (
